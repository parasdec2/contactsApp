export default function ($scope, $rootScope, $location, dbProvider) {
  if (!$rootScope.loggedIn && $rootScope.user === null) {
    ...
    $scope.deleteContact = function (id) {
      if (confirm("Do you want to delete this contact?")) {
        dbProvider.deleteContact(id).then((res) => {
          if (res === 200) {
            $scope.allContacts = $scope.allContacts.filter((e) => e._id !== id);
          }
        });
      }
    };
    ...
    $scope.updateContact = function () {
      ...
      dbProvider.updateContact(updatedContact, $scope.cId).then((contacts) => {
        $scope.cName = "";
        $scope.cEmail = "";
        $scope.cPhone = "";
        $scope.cId = "";
